...
export class EdittripPage implements OnInit {
  ...
  constructor(
    ...
    // this.editProvincesArray("Abucay")
  }
 ...
 // if the value is an empty string don't filter the items
  ...
  initDatePicker(instanceCalled) {
    ...
    if(instanceCalled == 0){
      ...
      // fromDate = new Date(this.getCurrentDate(new Date()))
    }else if(instanceCalled == 1){
      inputDate = new Date(this.toDate)
  ...
  onChangeFromDate(){
    ...
    console.log(this.fromDate.substring(0, this.fromDate.length - 1))
  }
  ...
  editProvincesArray(newProvince) {

    console.log(newProvince);
    console.log(this.oldprovince)
    return new Promise((resolve, reject) => {
      this.travelAppService.fetchVisitedProvicesArray().then(visitedArray => {
        this.testProvincesArray = visitedArray;
        console.log(this.testProvincesArray);

        for (var i = 0; i < this.testProvincesArray.length; i++) {
          if (this.oldprovince == this.testProvincesArray[i]) {
            this.testProvincesArray[i] = newProvince;
          }
        }
        console.log(this.testProvincesArray);
        firebase
          ...
          .update({
            visitedProvincesArray: this.testProvincesArray
          })
          .then(res => {
            resolve(res);
      ...
      // this.testProvincesArray.forEach(province =>{
      //   // if(this.testProvincesArray.includes())
      // })
    });
  }
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      if (this.router.getCurrentNavigation().extras.state) {
        ...
        this.fromDate2 = this.data.fromDate2;
        this.toDate2 = this.data.toDate2;
        this.tripuid = this.data.tripuid;
        this.province = this.data.province;
        this.city = this.data.city;
        this.province = this.data.province;
        this.oldprovince = this.data.province;
        this.eventTitle = this.data.eventTitle;
        this.aboutTrip = this.data.aboutTrip;
        this.base64TripPhoto = this.data.photo;

        this.states = this.getProvinces();

        this.cities = this.showCities(this.province);

        this.tomin = this.fromDate;

        this.provinceSearchTerm = this.province;
        this.citySearchTerm = this.city;
  ...
  uploadEdit() {
    if (this.province == "") {
      ...
      this.presentToast(
        "Title length is too short, input at least 3 characters"
      );
    } else if (this.aboutTrip == "") {
      this.presentToast("About trip field cannot be left blank");
    } else if (this.aboutTrip.length < 3) {
      this.presentToast(
        "About trip length is too short, input at least 3 characters"
      );
      ...
      this.loadingCtrl.create({ message: "Uploading your trip" }).then(res => {
        ...
        this.travelAppService
          ...
          .then(() => {
            this.editProvincesArray(this.province)
              .then(() => {
                res.dismiss();
                this.aboutTrip = "";
                this.city = "";
